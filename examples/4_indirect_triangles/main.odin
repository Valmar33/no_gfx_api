...
main :: proc()
{
    ...
    // Indirect draw command structure matching VkDrawIndexedIndirectCommand
    ...
    // Allocate indirect command buffer in CPU-visible memory (which is GPU-accessible)
    indirect_command_cpu_mem := gpu.mem_alloc_typed(Draw_Indexed_Indirect_Command, 1)
    defer gpu.mem_free_typed(indirect_command_cpu_mem)
    
    // Initialize indirect command
    indirect_command_cpu_mem[0] = Draw_Indexed_Indirect_Command {
        index_count = 3,
        instance_count = 1,
        first_index = 0,
        vertex_offset = 0,
